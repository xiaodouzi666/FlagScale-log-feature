...
class TestFLOPSFormulas(unittest.TestCase):
    ...
    def test_attention_flops(self):
        ...
        attention_score_flops = 2 * self.batch_size * self.num_heads * self.seq_length * self.seq_length * head_dim

        # scores @ V: 2 * batch * heads * seq * seq * head_dim
        attention_output_flops = 2 * self.batch_size * self.num_heads * self.seq_length * self.seq_length * head_dim

        # Output projection: 2 * batch * seq * hidden^2
        output_proj_flops = 2 * self.batch_size * self.seq_length * self.hidden_size * self.hidden_size

        expected_flops = qkv_flops + attention_score_flops + attention_output_flops + output_proj_flops
    ...
    def test_rotary_embedding_flops(self):
        ...
        elements = self.batch_size * self.seq_length * self.num_heads * (self.hidden_size // self.num_heads)
