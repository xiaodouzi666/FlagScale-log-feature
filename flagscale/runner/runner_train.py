...
def _generate_run_script_train(config, host, node_rank, cmd, background=True, with_test=False):
    ...
    with open(host_run_script_file, "w") as f:
        ...
        f.write(f'sys.path.insert(0, \"{root_dir}\")\n')
        f.write(f'from omegaconf import OmegaConf\n')
        f.write(f'from flagscale.runner.runner_train import SSHTrainRunner\n')
        f.write(f'from flagscale.elastic.monitor_service import MonitorService\n')
        ...
        f.write(f'        \"system\": {{\n')
        f.write(f'            \"logging\": {{\n')
        f.write(f'                \"log_dir\": \"{logging_config.log_dir}\"\n')
        f.write(f'            }}\n')
        f.write(f'        }}\n')
        f.write(f'    }},\n')
        f.write(f'    \"experiment\": {{\n')
        f.write(f'        \"runner\": {{\n')
        f.write(f'            \"no_shared_fs\": {str(no_shared_fs).lower()},\n')
        f.write(f'            \"ssh_port\": 22\n')
        f.write(f'        }}\n')
        f.write(f'    }}\n')
        f.write(f'}}\n')
        f.write(f'config = OmegaConf.create(config_dict)\n')
        f.write(f'\n')
        f.write(f'# 创建虚拟runner用于状态查询\n')
        f.write(f'class DummyRunner:\n')
        f.write(f'    def __init__(self):\n')
        f.write(f'        self.resources = None\n')
        f.write(f'        self.config = config\n')
        f.write(f'    def _query_status(self):\n')
        f.write(f'        import os\n')
        f.write(f'        from flagscale.runner.runner_base import JobStatus\n')
        f.write(f'        pid_file = \"{host_pid_file}\"\n')
        f.write(f'        if os.path.exists(pid_file):\n')
        f.write(f'            try:\n')
        f.write(f'                with open(pid_file, \"r\") as f:\n')
        f.write(f'                    pid = int(f.read().strip())\n')
        f.write(f'                import subprocess\n')
        f.write(f'                result = subprocess.run([\"ps\", \"-p\", str(pid)], capture_output=True)\n')
        f.write(f'                if result.returncode == 0:\n')
        f.write(f'                    return JobStatus.RUNNING\n')
        f.write(f'                else:\n')
        f.write(f'                    return JobStatus.COMPLETED_OR_IDLE\n')
        f.write(f'            except:\n')
        f.write(f'                return JobStatus.COMPLETED_OR_IDLE\n')
        f.write(f'        else:\n')
        f.write(f'            return JobStatus.COMPLETED_OR_IDLE\n')
        f.write(f'\n')
        f.write(f'runner = DummyRunner()\n')
        f.write(f'monitor = MonitorService(config, runner, interval=5)\n')
        f.write(f'monitor.start_monitoring(enable_log_collection=True, enable_diagnostic=True)\n')
        f.write(f'\n')
        f.write(f'print(\"Monitor service started for training process\")\n')
        f.write(f'" &\n')
        f.write(f'\n')
